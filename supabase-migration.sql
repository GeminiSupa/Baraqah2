...
-- Profiles are public for browsing
CREATE POLICY "Profiles are viewable by authenticated users" ON profiles
  FOR SELECT USING (profile_visibility = 'public' OR auth.uid()::text = user_id);

-- Messages are viewable by sender/receiver
...
-- Message requests are viewable by sender/receiver
CREATE POLICY "Message requests are viewable by participants" ON message_requests
  FOR SELECT USING (
    auth.uid()::text = sender_id OR 
    auth.uid()::text = receiver_id
  );
